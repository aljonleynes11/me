function calendarApp() {
    // Helper to get calendarId from URL
    function getCalendarId() {
        const params = new URLSearchParams(window.location.search);
        return params.get('calendarId');
    }
    // Helper to get calendar meta (name/desc) from calendarList
    function getCalendarMeta(calendarId) {
        const list = JSON.parse(localStorage.getItem('calendarList') || '[]');
        return list.find(c => c.id === calendarId) || { name: 'Calendar', description: '' };
    }
    const calendarId = getCalendarId();
    if (!calendarId) {
        window.location.href = 'index.html';
    }
    const meta = getCalendarMeta(calendarId);
    return {
        calendar: null,
        showOffcanvas: false,
        editingEvent: null,
        ...
        selectedDate: null,
        documentName: localStorage.getItem('documentName_' + calendarId) || meta.name || 'Calendar',
        editingDocumentName: false,
        descriptionTitle: localStorage.getItem('descriptionTitle_' + calendarId) || 'Description',
        editingDescriptionTitle: false,
        descriptionContent: localStorage.getItem('descriptionContent_' + calendarId) || meta.description || '',
        ...
        initCalendar() {
            ...
            const tz = localStorage.getItem('defaultTimezone') || 'Asia/Manila';
        ...
        loadEvents() {
            const stored = localStorage.getItem('calendarEvents_' + calendarId);
            this.events = stored ? JSON.parse(stored) : [];
        },

        saveEventsToStorage() {
            localStorage.setItem('calendarEvents_' + calendarId, JSON.stringify(this.events));
        },
        ...
        saveEvent() {
            ...
            // Remove note requirement: only check date, time, pnl
            if (!this.eventForm.date || !this.eventForm.time || !this.eventForm.pnl) {
                Swal.fire({
                    icon: 'warning',
        ...
        displayItemCountOnDay(arg) {
            ...
            if (dayEvents.length > 0) {
                ...
                // Determine background color and sign based on PnL
                let bgColor = 'bg-gray-500';
                let displayText = totalPnL.toFixed(2);
                if (totalPnL > 0) {
        ...
        finishEditingDocumentName() {
            ...
            localStorage.setItem('documentName_' + calendarId, this.documentName);
            document.title = this.documentName;
            // Also update in calendarList
            const list = JSON.parse(localStorage.getItem('calendarList') || '[]');
            const idx = list.findIndex(c => c.id === calendarId);
            if (idx !== -1) {
                list[idx].name = this.documentName;
        ...
        finishEditingDescriptionTitle() {
            ...
            localStorage.setItem('descriptionTitle_' + calendarId, this.descriptionTitle);
        },
        ...
        saveDescription() {
            if (window._quillDescription) {
                ...
                localStorage.setItem('descriptionContent_' + calendarId, this.descriptionContent);
                // Also update in calendarList
                const list = JSON.parse(localStorage.getItem('calendarList') || '[]');
                const idx = list.findIndex(c => c.id === calendarId);
                if (idx !== -1) {
                    list[idx].description = this.descriptionContent;
                    localStorage.setItem('calendarList', JSON.stringify(list));
                }
            }
        },
